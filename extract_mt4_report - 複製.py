...
def parse_period_info(period_str):
    """
    Parse the MT4 period string to extract period, start_date, end_date.
    Example input:
      '5 Minutes (M5)  2022.08.08 00:05 - 2025.08.06 02:55    (2022.08.07 - 2025.08.07)'
    Returns:
      period: 'M5'
      start_date: '2022.08.08'
      end_date: '2025.08.07'
    """
    # Find the text inside the first parenthesis (Period)
    m_period = re.search(r"\(([^)]+)\)", period_str)
    period = m_period.group(1) if m_period else ""

    # Find the first date after the parentheses (start date)
    m_start = re.search(r"\)\s+(\d{4}\.\d{2}\.\d{2})", period_str)
    start_date_str = m_start.group(1) if m_start else ""

    # Find the last date in the string (inside the last parenthesis)
...
def parse_metrics(html_string):
    ...
    # --- Parse period info and add to metrics ---
    ...
    # Multi-metric rows: scan for rows where the first cell is "Largest", "Average", "Maximum", or "Maximal"
    for row in table.find_all("tr"):
        tds = row.find_all("td")
    ...
    # Special: "Average consecutive wins/losses" row
    for row in table.find_all("tr"):
        tds = row.find_all("td")
        texts = [td.get_text(strip=True) for td in tds]
    ...
    # The rest (composite parse for special fields)
    ...
    # Largest/Average profit/loss as float for custom metrics
...
def parse_trades(html_string):
    ...
    if len(tables) < 2:
        return []
    trade_table = tables[-1]  # Use last table by default (robust)
    trades = []
    headers = []
...
def calculate_custom_metrics(metrics, trades):
    ...
    # Use parsed value for max_drawdown and max_drawdown_pct
    ...
    # New fields for previously missing metrics
...
def generate_summary_insert(step_id, metric_type, metrics, custom_metrics, parameters_json, summary_csv):
    ...
    max_consecutive_loss, max_consecutive_loss_count, metrics_json, parameters_json, summary_csv, start_date, end_date
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
"""
    values = [
        step_id,
...
def gen_summary_csv(metrics, custom_metrics, csv_name):
    """
    Generate a summary CSV file (standard, human-readable) and return the summary_csv string.
    Excludes the 'Parameters' field.

    Args:
        metrics (dict): Dictionary containing parsed metrics.
        custom_metrics (dict): Dictionary containing calculated custom metrics.
        csv_name (str): Name of the output CSV file.

    Returns:
        str: The generated summary CSV string (header + values, separated by newlines).
    """
    # Set of all unique keys, custom_metrics first, and drop 'Parameters'
...
def process_mt4_report(
    ...
    # Prepare parameters JSON and summary CSV (excluding "Parameters" column)
    ...
    # --- Build output set filename and magic number using build_filename ---
    # Use EA_name and metrics
    # For build_filename, we need:
    # EA, Symbol, Timeframe, InitialDeposit, ProfitAmount, DrawDown, StartDate, EndDate, Stoploss, WinRate, ProfitFactor, NumTrade, SetVersion, Step, ext
    # Get required fields, fallback to ""
    ...
    DrawDown = metrics.get("Maximal drawdown pct", "")
    # Dates as string YYYYMMDD
    ...
    if EndDate:
        ...
        EndDate = ""
    Stoploss = ""  # You can extract from Params if needed
    ...
    # Suppose output_set_file_name = "MyEA_EURUSD_M5_20220808-20250807.set"
    output_set_name_no_ext = os.path.splitext(output_set_file_name)[0]
    summary_metrics_full_path = os.path.join(
        output_set_file_path,
        f"{output_set_name_no_ext}_summary_metrics.csv"
    )

    # print("output_set_file_path:", repr(output_set_file_path))
    # print("output_set_file_name:", repr(output_set_file_name))
    # print("summary_metrics_full_path:", repr(summary_metrics_full_path))

    # Then use:
    summary_csv = gen_summary_csv(metrics_no_parameters, custom_metrics, summary_metrics_full_path)

    # --- Copy input set file to output file ---
    shutil.copy(input_set_file, output_file)
    # --- Update Magic parameter ---
    ...
    result = "success"
    print(result)  # For UiPath acknowledgement
    return result
...
if __name__ == "__main__":
    ...
    parser.add_argument("--summary_metrics_path", type=str, default="summary_metrics.csv", help="Path to output summary metrics CSV file")
...
#
#     parameters_json = json.dumps({"Parameters": metrics.get("Parameters", "")})
#     summary_csv = gen_summary_csv(metrics, custom_metrics, "summary_metrics.csv")
#
#     summary_sql, summary_values = generate_summary_insert(
#         step_id, metric_type, metrics, custom_metrics, parameters_json, summary_csv
#     )
...
#
#     print("-- Summary Metrics SQL:")
#     print(summary_sql)
#     print("-- Values:", summary_values)
#
#     trade_sql, trade_values = generate_trade_inserts(
#         step_id, trades, metrics.get("Symbol", "")
#     )
#     print("-- Trades SQL:")
#     print(trade_sql)
#     print("-- Number of Trades:", len(trade_values))
#     print("-- First trade values:", trade_values[0] if trade_values else "None")
#
#     print("Columns in SQL:", summary_sql.count(',') + 1)
#     print("Values in values list:", len(summary_values))
#
#     print("SQL columns:", summary_sql[summary_sql.find("(") + 1:summary_sql.find(")")].replace('\n', '').split(','))
#     print("Values:", len(summary_values), summary_values)
#
#     # Database connection
