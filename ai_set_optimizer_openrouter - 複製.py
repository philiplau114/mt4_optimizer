...
def build_prompt_with_history(
    ...
    wave_analysis_result_block    # <--- NEW
):
    prompt = prompt_template.format(
        ...
        wave_analysis_result_block=wave_analysis_result_block   # <--- NEW
    )
...
def extract_json_objects_from_response(text):
    ...
    for block in code_blocks:
        ...
        try:
            ...
            continue
    logger.info(f"Extracted {len(results)} JSON objects from response.")
    return results
...
#                 logger.error(f"No valid suggestion JSON found from {model}. Raw response written to: {suggestions_path}")
#             except Exception as e:
#                 logger.error(f"Failed to write raw response to {suggestions_path}: {e}")
#             continue
...
def suggest_mode_and_sections_and_params_openrouter(
    ...
    prompt_save_path = suggestion_json_path.replace(".json", ".prompt.txt")
