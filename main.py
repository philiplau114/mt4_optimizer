...
def parse_section_args(section_arg):
    """
    Parse comma-separated section argument string into a list, stripping whitespace.
    """
    return [s.strip() for s in section_arg.split(',')] if section_arg else []

def prompt_openai_for_suggestions(param, set_content, htm_content, api_key, current_value=None, ignore_sections=None, suggest_sections=None):
    ...
    prompt = (
        ...
        "When suggesting parameters to optimize, do not include any parameters that are under sections you should ignore, as specified below.\n\n"
        ...
        f"\nAlways include '{param}' in your suggestions, even if it is not in the suggested sections."
    )
    if ignore_sections:
        prompt += f"\nIgnore suggesting parameters from the following sections: {', '.join(ignore_sections)}."
    if suggest_sections:
        prompt += f"\nOnly suggest parameters from the following sections: {', '.join(suggest_sections)}."
    ...
    prompt += (
        f"\nAdditionally, please review the file and suggest any other important parameters to optimize for improved performance, but exclude any from the ignore sections. "
    ...
    # Guarantee param is always present in result
    found_param = any(p.get("name", "").strip() == param for p in params)
    if not found_param:
        # Add param suggestion with current_value if missing
        param_obj = {"name": param, "start": current_value if current_value is not None else "", "end": current_value if current_value is not None else "", "step": ""}
        params.append(param_obj)
    return params, resp

def get_file_head(path, max_chars=4000):
    try:
...
def main():
    ...
    parser.add_argument("--ignore-sections", required=False, help="Comma-separated list of sections to ignore")
    parser.add_argument("--suggest-sections", required=False, help="Comma-separated list of sections to suggest")
    ...
    ignore_sections = parse_section_args(args.ignore_sections)
    suggest_sections = parse_section_args(args.suggest_sections)

    suggestions, raw_response = prompt_openai_for_suggestions(
        args.param, set_content, htm_content, args.api_key,
        current_value=current_value, ignore_sections=ignore_sections, suggest_sections=suggest_sections
    )
